 ...
 * @since 3.9.3
 */
function mphb_filter_edit_page_field_groups_value( $fieldGroups ) {
	foreach($fieldGroups as $group) {
		if( $group->getName() == 'mphb_internal_notes' ) {
			if(empty($group->getValues())) {
				$_POST['_mphb_booking_internal_notes'] = array();
			}
		}
	}
	return $fieldGroups;
}
add_filter( 'mphb_edit_page_field_groups', 'mphb_filter_edit_page_field_groups_value' );
